    ...
    if os.path.isdir(path + "/" + file):
        ...
        for sub_file in sub_files:
            ...
            if ".pickle" in sub_file and "records" in sub_file and "passenger" not in sub_file:
                ...
                print("==========")
                print("司机数量：",driver_num)
                print("匹配率：", get_matching_rate(records, order))
                print("乘客平均等待时间：",get_avg_prematching_waiting_time(records,order))
                print("乘客平均接单时间：",get_postmatching_pickup_time(records))
                print("司机接单率：",get_driver_pickup_ratio(records,36000,79200,500))
                print("司机送客时间：",get_driver_delivery_time(records,start_time,end_time,driver_num))
                print("司机利用率：",get_driver_delivery_ratio(records,start_time,end_time,driver_num))
                print("matching radius:",env_params['broadcasting_scale'])
                #statistic.plot_figure(order)
