...
if __name__ == "__main__":
    ...
    train_set_result = pd.DataFrame(
        columns=['trip_distance', 'wait_time', 'start_time', 'origin_lat', 'origin_lng', 'busy_ratio', 'radius',
                 'reward'])
    radius_model = MLP_nn()
    pred_list = [2.601150, 5, 36005, 40.7679674, -73.9682154, 27.7, 8]
    print('预测结果：', radius_model.predict([pred_list]))
    # for i in radius_list:
    #     env_params['broadcasting_scale'] = i
                    ...
                    for single_max_distance_num in max_distance_num:
                        ...
                        pickle.dump(match_and_cancel_track_list,
                                    open(file_path + '/match_and_cacel_' + str(single_driver_num) + '.pickle',
                                         'wb'))
                        # list1 = pickle.load(
                        #     open(file_path + '/records_driver_num_' + str(single_driver_num) + '.pickle', 'rb'))
                        # list2 = pickle.load(
                        #     open(file_path + '/passenger_records_driver_num_' + str(single_driver_num) + '.pickle',
                        #          'rb'))
                        # list3 = pickle.load(
                        #     open(file_path + '/match_and_cacel_' + str(single_driver_num) + '.pickle', 'rb'))
                        # print('record =', list1)
                        # print('passenger =', list2)
                        # print('match cancel =', list3)
                        file = open(file_path + '/time_statistic.txt', 'a')
                        file.write(str(time.time() - t) + '\n')
                        # train_set.insert(train_set.shape[1], '%s' % config.env_params['broadcasting_scale'],
                        #                  list(simulator.reward['reward']))
                        #print(simulator.matched_requests)
                        ...
                        # train_set['start_time'] = simulator.matched_requests['start_time']
                        # train_set['origin_lat'] = simulator.matched_requests['origin_lat']
                        # train_set['origin_lng'] = simulator.matched_requests['origin_lng']
                        ...
                        #
                        # train_set_result.to_csv("train.csv", mode='w', index=True, sep=',')
