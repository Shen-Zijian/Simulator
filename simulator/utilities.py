...
"""
# key:current_time_period, value:

time_params_dict = {'morning': [2.582, 2.491, 0.026, 1.808, 2.581],
...
"""
# myclient = pymongo.MongoClient("mongodb://localhost:27017/","147.8.128.123")
...
def skewed_normal_distribution(u, thegma, k, omega, a, input_num):
    input_num = (input_num - k) / omega
    a1 = st.norm.cdf(a * input_num, loc=u, scale=thegma)  # cumulative distribution
    a2 = st.norm.pdf(input_num, loc=u, scale=thegma)  # probability distribution
    res_snd = 2 * a1 * a2
    return res_snd
...
def get_coordinate_from_nodeId(node_id):
    """

    :param id: list,id of driver/order
    :return: the coordinate
    """
    result = []
    for i in range(len(node_id)):
        node_id_to_lat_lng[node_id[i]] = (lat_list[i], lng_list[i])
        # print("res =",type(node_id_to_lat_lng[node_id[i]]))
        result.append(node_id_to_lat_lng[node_id[i]])
    # print("res =",result)
    return result
...
def get_nodeId_from_coordinate(lat, lng):
    ...
    """
    node_list = []
    # print("len of lat =", len(lat_list))
    # print("len of lng =", len(lng_list))
